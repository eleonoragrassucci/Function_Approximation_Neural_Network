...
def error(pars, args, params = 1):
    ...
    if params == 0:
        ...
        return (1/(2*P))*(np.linalg.norm(y_mlp - y))**2 + (rho*(np.linalg.norm(v))**2)
...
def best_values (x_train,y_train,x_test,y_test,N=21,rho=0.00001, ini=15):
    ''' x_train = x_train set
        y_train = y_train set
        x_test = x_test set
        y_test = y_test set
        ...
        ini = number of repetitions to reduce the beginnings with "bad points", def=15
    '''
    ...
    for _ in range(ini):
        ...
        err_args = [N, x_train, y_train, rho,w_init,b_init]
    ...
    paramss = [results[N,rho]["v"]]
    paramss = list(itertools.chain(*paramss))
    err_argss = [N, x_test, y_test, rho,w_init,b_init]
...
print("But we repeat code 15 times for avoiding 'Bad Starts', so time for optimizing the network:... %s seconds" %((results[4]/results[6])))
